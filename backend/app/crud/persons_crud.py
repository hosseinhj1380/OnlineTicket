...
class PersonsCRUD:
        ...
        try:
            ...
            for role in PersonInfo.get("roles"):
                if check_role(role=role.get("name"), id=role.get("id")):
                    ...
                    return {"status": "success", "message": "valueerror"}
            persons_collection.insert_one(PersonInfo)

            return {"status": "success", "message": "person saved successfully "}
    ...
    def get(self, PersonID):
        person_info = persons_collection.find_one(
            {"PersonID": PersonID}, {"_id": False}
        )
        if person_info:
            return person_info
        else:
        ...
        if persons_collection.find_one({"PersonID": PersonID}, {"_id": False}):
            try:
                persons_collection.update_one(
                    {"PersonID": PersonID}, {"$set": PersonInfo}
                )
                return {"message": "success", "PersonInfo": PersonInfo}
            except Exception as e:
                return e
        else:
            return None
...
def check_user_ID(id):
    person = persons_collection.find_one(
        {"PersonID": id},
        {"full_name": True, "profile_photo": True, "PersonID": True, "_id": False},
    )
    if person:
        return person
    else:
